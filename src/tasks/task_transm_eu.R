#============================================================================
# Task: Download EU cross-border transmission flows from ENTSO-E
#         - Scheduled commercial exchange
#         - Physical cross-border flow
# Uses smaller chunk_days (90) to avoid ENTSO-E timeouts on many zone pairs
#============================================================================

task_transm_eu <- function(end_date = lubridate::today() - lubridate::days(1)) {
  ...
  # chunk_days = 90 to avoid timeouts when querying 8 zone pairs per chunk
  results$transm_sched <- run_subtask(
    "Scheduled exchange EU", "data/data_raw/transm_sched_EU.csv",
    download_fn = function(s, e) download_transm_sched_eu(zone_pairs, s, e, chunk_days = 90)
  )

  results$transm_phys <- run_subtask(
    "Physical flow EU", "data/data_raw/transm_phys_EU.csv",
    download_fn = function(s, e) download_transm_phys_eu(zone_pairs, s, e, chunk_days = 90)
  )
