...
```{r}
sched_capacity <- read_csv("/Users/andrii/Documents/R files/_My projects/RES data download/data/data_raw/transm_sched_EU.csv")
flows <- read_csv("/Users/andrii/Documents/R files/_My projects/RES data download/data/data_raw/transm_phys_EU.csv")
...
flows |> 
  ...
  summarise(
    flow = sum(physical_flow_mw, na.rm = TRUE),
    .groups = "drop"
  ) |> 
    ggplot(aes(x = date, y = ifelse(direction == "export", flow, -flow), fill = country)) +
    geom_col(stat = "identity", alpha = 0.7, color = NA) +
...
total_flows <- flows |> 
  mutate(direction = ifelse(from_country == "UA", "export", "import"),
    country = ifelse(direction == "export", to_country, from_country)) |> 
  group_by(direction, date = floor_date(hour, unit = "days")) |>
  summarise(
    flow = sum(physical_flow_mw, na.rm = TRUE),
    .groups = "drop"
  ) 

total_flows_net <- total_flows |> 
  pivot_wider(
...
ggplot() +
  geom_col(data = total_flows, 
    aes(x = date, y = ifelse(direction == "export", flow, -flow), fill = direction),
    stat = "identity", 
    position = "dodge", 
    alpha = 0.7, 
    color = NA) +
  geom_line(data = total_flows_net, aes(x = date, y = net), color = "black", size = 0.5) +
  geom_hline(yintercept = 0, color = "grey40") +
  theme_minimal()
...
sched_capacity |> 
  ...
  group_by(direction, country, date = floor_date(hour, unit = "months")) |>
  summarise(
    flow = sum(scheduled_mw, na.rm = TRUE),
    .groups = "drop"
  ) |> 
    ggplot(aes(x = date, y = ifelse(direction == "export", flow, -flow), fill = country)) +
    geom_col(stat = "identity", alpha = 0.7, color = NA) +
...
ggplot() +
  geom_col(data = total_sched, 
    aes(x = date, y = ifelse(direction == "export", flow, -flow), fill = direction),
    stat = "identity", 
    position = "dodge", 
    alpha = 0.7, 
    color = NA) +
  geom_line(data = total_sched_net, aes(x = date, y = net), color = "black", size = 0.5) +
  geom_hline(yintercept = 0, color = "grey40") +
  labs(title = "Scheduled Capacity") +
  theme_minimal()
```